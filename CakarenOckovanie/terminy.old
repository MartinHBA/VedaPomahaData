...
$versionsCheck = git rev-list --all --reverse -- OpenData_Slovakia_Vaccination_Scheduled.csv
...
foreach ($item in $versionsCheck) {
    $SEL = Select-String -Path "D:\OneDrive\git\VedaPomahaData\CakarenOckovanie\versionlistScheduled.txt" -Pattern $item
...
foreach ($item in $collection) {
    $i++
    $filename = "$($i).csv"
    $uri = "https://github.com/Institut-Zdravotnych-Analyz/covid19-data/raw/$($item)/Vaccination/OpenData_Slovakia_Vaccination_Scheduled.csv"
    $result = Invoke-WebRequest -Uri $uri -Method GET
    $result.Content | Out-File -FilePath "D:\OneDrive\git\VedaPomahaData\CakarenOckovanie\dataScheduled\$($filename)" -Encoding "utf8"
...
if ($i) {
    ...
    $env:PATHEXT += ";.py"; 
    $arg1 = 'D:\OneDrive\git\VedaPomahaData\CakarenOckovanie\PythonScript\calculateTrendScheduled.py' 
    $arg2 = $i
    python $arg1 $arg2
}
